...
export class MediaManager {
  constructor(initialValues) {
     ...
     */
    this.localVideo =
      this.localVideoElement || document.getElementById(this.localVideoId);
        ...
        (audioStream) => {
          ...
          if (this.publishMode == "screen") {
            ...
            return this.updateAudioTrack(audioStream, streamId, null).then(
              () => {
                return this.setDesktopwithCameraSource(
                  stream,
                  streamId,
                  onended
                );
  ...
  getMedia(mediaConstraints, audioConstraint, streamId) {
    ...
    // If mediaConstraints only user camera
      ...
      return this.navigatorUserMedia(mediaConstraints)
        .then((stream) => {
          if (this.smallVideoTrack) this.smallVideoTrack.stop();
          return this.prepareStreamTracks(
            mediaConstraints,
  ...
  mixAudioStreams(stream, secondStream) {
    ...
    if (stream.getAudioTracks().length > 0) {
      ...
      audioSource
        .connect(this.primaryAudioTrackGainNode)
    ...
    if (secondStream.getAudioTracks().length > 0) {
      ...
      audioSource2
        .connect(this.secondaryAudioTrackGainNode)
        .connect(audioDestionation);
    } else {
  ...
  switchVideoCameraCapture(streamId, deviceId, onEndedCallback) {
    ...
    return navigator.mediaDevices.enumerateDevices().then((devices) => {
      ...
      return this.setVideoCameraSource(
        streamId,
        this.mediaConstraints,
        null,
        true,
  ...
  switchVideoCameraFacingMode(streamId, facingMode) {
    ...
    this.publishMode = "camera";
    console.debug(
      "Media constraints video property = " + this.mediaConstraints.video
    );
    return this.setVideoCameraSource(
      streamId,
  ...
  applyConstraints(streamId, newConstraints) {
    var constraints = {};
    if (
      newConstraints.audio === undefined &&
      newConstraints.video === undefined
    ) {
    ...
    if (constraints.audio !== undefined) {
      ...
      promise = this.setAudioInputSource(
        streamId,
